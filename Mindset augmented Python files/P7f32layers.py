...
# Save Everything 
torch.save(model.state_dict(), "segmentation_pointnet_f32layers.pth")
...
model.eval()
with torch.no_grad():
    test_out = model(X_test).permute(0, 2, 1)  # shape: [batch, classes, points]
    test_losses = loss_fn(test_out, y_test)   # shape: [batch, points] due to reduction="none"
    per_point_loss = test_losses.cpu().numpy()                       # [batch, points]
    per_model_loss = test_losses.mean(dim=1).cpu().numpy()          # [batch] average loss per diagram
    cost_test = test_losses.mean().item()                            # scalar average loss
    test_predictions = test_out.argmax(dim=1).cpu().numpy()         # [batch, points]
    test_targets = y_test.cpu().numpy()                             # [batch, points]
...
with open("output_f32layers.pkl", "wb") as f:
    pickle.dump(test_out, f)
with open("cost_f32layers.pkl", "wb") as f:
    pickle.dump(cost_test, f)
with open("test_targets_f32layers.pkl", "wb") as f:
    pickle.dump(test_targets, f)
with open("test_predictions_f32layers.pkl", "wb") as f:
    pickle.dump(test_predictions, f)
with open("per_point_losses_test_f32layers.pkl", "wb") as f:
    pickle.dump(per_point_loss, f)
with open("average_loss_per_model_f32layers.pkl", "wb") as f:
    pickle.dump(per_model_loss, f)
with open("test_accuracy_f32layers.pkl", "wb") as f:
