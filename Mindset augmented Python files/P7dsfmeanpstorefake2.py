...
for epoch in range(Nepoch):
    ...
    for i in range(nbatches):
        ...
        for j in range(x_batch.shape[0]):
            ...
            xj, yj, _ = inject_fake_frequencies(xj, yj)
...
with torch.no_grad():
    ...
    for i in range(X_test.shape[0]):
        x_i = X_test[i]
        y_i = y_test[i]
        x_aug, y_aug, dropped_i, shake_i = apply_dropper_and_shaker(x_i, y_i)
        x_aug, y_aug, fake_i = inject_fake_frequencies(x_aug, y_aug)
        fake_probs = None
        if fake_i is not None:
            logits = model(x_aug.unsqueeze(0))
            probs = F.softmax(logits, dim=2)[0]
            fake_probs = probs[x_i.shape[0]:, :].cpu().numpy()
        dsf_extra_info_test[f"test_model_{i}"] = {
            "dropped_indices": dropped_i.cpu().numpy(),
            "fake_points": fake_i.cpu().numpy() if fake_i is not None else None,
            "fake_probs": fake_probs
        }

torch.save(model.state_dict(), "segmentation_pointnet_dsfmeanpstorefake2.pth")
with open("output_logits_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(test_out.cpu().numpy(), f)
with open("output_probs_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(test_probs, f)
with open("cost_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(cost_test, f)
with open("test_targets_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(test_targets, f)
with open("test_predictions_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(test_predictions, f)
with open("per_point_losses_test_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(per_point_loss, f)
with open("average_loss_per_model_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(per_model_loss, f)
with open("test_accuracy_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(test_accuracy, f)
with open("dsf_extra_info_test_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(dsf_extra_info_test, f)
with open("train_history_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(history_train, f)
with open("val_history_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(history_test, f)
with open("val_acc_history_dsfmeanpstorefake2.pkl", "wb") as f: pickle.dump(history_test_acc, f)
