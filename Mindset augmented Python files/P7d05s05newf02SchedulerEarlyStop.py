...
SUFFIX = "d05s05newf02SchedulerEarlyStop"  # <--- Change suffix here for all saved files
FAKE_CLASS = 3
DROPPED_CLASS = -1
DROP_PROB = 0.5
SHAKE_STD = 0.05
...
for epoch in range(Nepoch):
    ...
    for i in range(nbatches):
        ...
        # Inject fake freqs and apply dropper/shaker per sample
        ...
        loss = loss_fn_train(seg_output.permute(0, 2, 1), y_padded)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        cost_test = loss_fn_train(test_out.permute(0, 2, 1), y_test).mean()
    history_train.append(avg_train_loss)
    history_test.append(cost_test.item())

    print(f"Epoch {epoch+1}: Train Loss = {avg_train_loss:.4f}, Test Loss = {cost_test:.4f}")
