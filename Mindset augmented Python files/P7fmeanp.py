...
mod_frequencies = X_frequencies % delta_nu_values                                  # same shape as X_frequencies
...
def inject_fake_frequencies(x, y, max_fake_fraction=MAX_FAKE_FRACTION):
    """
    x: [N, 2], y: [N]
    - Keeps only positions where y != DROPPED_CLASS to recompute Δν.
    - Generates 'fake' points across the real freq range with small jitter.
...
for epoch in range(Nepoch):
    ...
    for i in range(nbatches):
        optimizer.zero_grad()
        # guard for last (possibly smaller) batch
        i0 = i * batch_size
        i1 = min((i + 1) * batch_size, X_train.shape[0])
        if i0 >= i1:
            continue

        x_batch = X_train[i0:i1]   # [b, N, 2]
        y_batch = y_train[i0:i1]   # [b, N]

        # Augment each sequence with fakes
        x_aug_list, y_aug_list = [], []
        for j in range(x_batch.shape[0]):
            xj, yj = inject_fake_frequencies(x_batch[j], y_batch[j], max_fake_fraction=MAX_FAKE_FRACTION)
        ...
        # Forward + loss
        out = model(x_padded)                          # [B, N, C]
        out_nll = out.permute(0, 2, 1)                 # [B, C, N]
        loss_mat = loss_fn(out_nll, y_padded)          # [B, N]
        loss = loss_mat.mean()
        loss.backward()
        optimizer.step()
    ...
    model.eval()
    with torch.no_grad():
        test_out = model(X_test)                       # [B, N, C]
        test_out_nll = test_out.permute(0, 2, 1)       # [B, C, N]
        test_loss_mat = loss_fn(test_out_nll, y_test)  # [B, N]
        val_loss = test_loss_mat.mean()

        # Train accuracy (from last train batch)
        train_preds = out.argmax(dim=2)                # [B, N]
        ...
        # Test accuracy
        test_preds = test_out.argmax(dim=2)            # [B, N]
    ...
    print(f"Epoch {epoch:3d}: "
          f"Train Loss = {float(loss):.4f}, Test Loss = {float(val_loss):.4f}, "
          f"Train Acc = {train_acc:.4f}, Test Acc = {test_acc:.4f}")

# Save everything
torch.save(model.state_dict(), "segmentation_pointnet_fmeanp.pth")

with open("train_history_fmeanp.pkl", "wb") as f:
    pickle.dump(history_train, f)
with open("test_history_fmeanp.pkl", "wb") as f:
    pickle.dump(history_test, f)
with open("train_acc_history_fmeanp.pkl", "wb") as f:
    pickle.dump(history_train_acc, f)
with open("test_acc_history_fmeanp.pkl", "wb") as f:
...
with open("output_logits_fmeanp.pkl", "wb") as f:
    pickle.dump(test_out.cpu(), f)
with open("cost_fmeanp.pkl", "wb") as f:
    pickle.dump(cost_test, f)
with open("test_targets_fmeanp.pkl", "wb") as f:
    pickle.dump(test_targets, f)
with open("test_predictions_fmeanp.pkl", "wb") as f:
    pickle.dump(test_predictions, f)
with open("per_point_losses_test_fmeanp.pkl", "wb") as f:
    pickle.dump(per_point_loss, f)
with open("average_loss_per_model_fmeanp.pkl", "wb") as f:
    pickle.dump(per_model_loss, f)
with open("test_accuracy_fmeanp.pkl", "wb") as f:
